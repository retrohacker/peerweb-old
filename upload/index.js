var orderButtons = '<i class="fa fa-arrow-up fa-2x move_up"></i><i class="fa fa-arrow-down fa-2x move_down"></i>'
var file_uuid = 0
var rootFolder = {}
...
function renderSubTree (root) {
  ...
  if (root.uuid !== 0) html += '<i class="fa fa-folder fa-2x"></i><input type="text" value="' + root.name + '" class="folderName" oninput="update_name(this)" />' + orderButtons
  else html += ''
  for (var i = 0; i < root.children.length; i++) {
    var child = root.children[i]
    if (child.children) html += renderSubTree(child)
    else html += '<div id="' + child.uuid + '" class="file"><i class="fa fa-file fa-2x"></i><input type="text" value="' + child.name + '" oninput="update_name(this)" class="fileName"/>' + orderButtons + '</div>'
  }
  html += '</div>'
  return html
}
...
function create_torrent () { // eslint-disable-line no-unused-vars
  client.seed(getFileList(), { name: '/' }, function (torrent) {
    console.log(torrent)
    document.getElementById('hash').innerHTML = '<b>Torrent:</b> <i>' + torrent.infoHash + '</i>'
  })
}
